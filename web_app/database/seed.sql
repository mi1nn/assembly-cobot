...
-- NOTE:
-- Post 작업은 DB parameter/components 값을 Robot Motion에 직접 전달한다.
-- postA~postF의 Force Place 파라미터는 아래 두 값으로 통일한다.
--   place_force           : TOOL -Z Desired Force = 30N
--   place_force_threshold : Place 완료 판단 |F_tool_z| >= 20N
-- place_retreat_distance는 Force 판정값이 아니라 Place 완료 후 이탈 거리이다.
-- 실제 설치 좌표가 확정되면 각 Operation의
-- pickup_position / assembly_position만 개별 수정한다.
...
'{"tcp":{"x":0,"y":0,"z":150},"ucs":null,"tool":"gripper_post","fixture":"jig_post","coordinate_system":"BASE","speed":100,"acceleration":200,"pick_distance":50,"place_retreat_distance":50,"place_force":30.0,"place_force_threshold":20.0}',
...
INSERT INTO robot (
    robot_id, robot_code, name, manufacturer,
    model, status, dofs, payload_kg
)
VALUES (
    1, 'RB-01', '두산 M0609 조립 로봇',
    'Doosan Robotics', 'M0609', 'IDLE', 6, 6.0
);


INSERT INTO sensor (
    sensor_id, robot_id, name, type,
    force_max_n, torque_max_nm, is_active
)
VALUES (
    1, 1, 'F/T 센서 1호', 'FORCE_TORQUE',
    500, 50, TRUE
);


INSERT INTO work_order (
    work_order_id, order_number, title, installation_id,
    priority, status, planned_start_date, planned_end_date,
    remark, created_by
)
VALUES (
    1, 'WO-20260821-001', '태양광 발전 시설 A 설치 작업',
    1, 1, 'COMPLETED',
    '2026-08-19 08:00:00', '2026-08-21 18:00:00',
    '옥상 크레인 협조 하에 진행', '관리자'
);


INSERT INTO work_execution (
    work_execution_id, work_order_id, robot_id,
    execution_number, status, start_time, end_time
)
VALUES (
    1, 1, 1, 'EX-20260821-001', 'COMPLETED',
    '2026-08-21 09:00:00', '2026-08-21 11:35:00'
);


INSERT INTO operation_execution (
    operation_execution_id, work_execution_id, operation_id,
    sequence, status, start_time, end_time, retry_count
) VALUES
...
INSERT INTO log (
    log_id, work_execution_id, operation_execution_id, robot_id,
    log_type, code, severity, message, detail, timestamp
) VALUES
...
(6,1,8,1,'ERROR','ERR-TORQUE-HIGH','ERROR',
 '패널 체결 중 토크 상한 근접 (측정 43.2Nm / 한계 45Nm)',
 '{"error_type":"TORQUE_LIMIT","is_resolved":true,"resolved_at":"2026-08-21T10:55:00","resolved_by":"김현장"}',
 '2026-08-21 10:52:19');


INSERT INTO sensor_data (
    sensor_data_id, sensor_id, operation_execution_id,
...
SELECT setval(
    pg_get_serial_sequence('installation','installation_id'),
    (SELECT MAX(installation_id) FROM installation)
);

SELECT setval(
    pg_get_serial_sequence('operation','operation_id'),
    (SELECT MAX(operation_id) FROM operation)
);

SELECT setval(
    pg_get_serial_sequence('robot','robot_id'),
    (SELECT MAX(robot_id) FROM robot)
);

SELECT setval(
    pg_get_serial_sequence('sensor','sensor_id'),
    (SELECT MAX(sensor_id) FROM sensor)
);

SELECT setval(
    pg_get_serial_sequence('work_order','work_order_id'),
    (SELECT MAX(work_order_id) FROM work_order)
);

SELECT setval(
    pg_get_serial_sequence('work_execution','work_execution_id'),
    (SELECT MAX(work_execution_id) FROM work_execution)
);

SELECT setval(
    pg_get_serial_sequence('operation_execution','operation_execution_id'),
    (SELECT MAX(operation_execution_id) FROM operation_execution)
);

SELECT setval(
    pg_get_serial_sequence('log','log_id'),
    (SELECT MAX(log_id) FROM log)
);

SELECT setval(
    pg_get_serial_sequence('sensor_data','sensor_data_id'),
    (SELECT MAX(sensor_data_id) FROM sensor_data)
);
