...
| DR-FR-42 | Component Position | 부품/자재의 현재 위치(픽업 위치)와 조립 위치를 관리할 수 있어야 한다. (구현: `operation.parameter`/`operation.components` JSONB 내부 필드로 관리) |
...
DR-FR-16 ~ DR-FR-20은 `operation.parameter` (JSONB)로 관리한다.
...
| DB-FR-36 | Sensor Result | 측정 데이터에 대한 작업 결과 또는 판정 정보를 관리할 수 있어야 한다. (⚠ `sensor_data` 테이블에 판정 결과 컬럼 없음 — `data` JSONB 원시 데이터만 저장) |
...
| ID | 요구사항 |
| --- | --- |
| DB-FR-37 | 시스템은 각 Operation의 실행 이력을 Work Order와 연결하여 조회할 수 있어야 한다. |
| DB-FR-38 | 시스템은 작업 결과와 발생한 이벤트 및 오류를 해당 Execution과 연결할 수 있어야 한다. |
| DB-FR-39 | 시스템은 측정 결과 및 센서 데이터를 해당 작업 실행과 연결하여 조회할 수 있어야 한다. |
| DB-FR-40 | 시스템은 완료된 작업의 주요 이력을 조회할 수 있어야 한다. |
